 ...
 *返回：数据库对象（sqlite3对象）
 *******************************/
+ (sqlite3 *) openDatabaseWithName: ()databaseName
{
 ...
 *返回：封装成数组的查询数据
 *******************************/
+ (NSMutableArray *) queryInfoWithDataBase: (sqlite3 *) database
                            WithSQL: (NSString *) SQL
{
    ...
    //通过预编译语句获取数据
    NSMutableArray *data = [self fetchDataWithStatement:statement];
 ...
 *返回：以数组的形式返回查询出来的数据
 *******************************/
+ (NSMutableArray *) fetchDataWithStatement: (sqlite3_stmt *) statement{
    ...
    return [NSMutableArray arrayWithArray:rowArray];
}
...
+(NSString*)queryInsertIntoTable:(NSString *)table withNumber:(NSString *)number andName:(NSString *)name andclassnumber:(NSString*)classnumber
{
    NSString* sql = [[NSString alloc] initWithFormat:@"INSERT INTO %@ VALUES ('%@','%@','%@')",table,number,name,classnumber];
    return sql;
}
+(NSString*)queryDeleteFromTable:(NSString *)table usingName:(NSString *)name
{
    NSString* sql = [[NSString alloc] initWithFormat:@"DELETE FROM %@ WHERE stuName = '%@'",table,name];
    return sql;
}
+(NSString*)queryDeleteFromTable:(NSString *)table usingNumber:(NSString *)number
{
    NSString* sql = [[NSString alloc] initWithFormat:@"DELETE FROM %@ WHERE stuNumber = '%@'",table,number];
    return sql;
}
+(NSString*)queryUpdateColumn:(NSString *)updatedColumn inTable:(NSString *)table withValue:(NSString *)value using:(NSString*)column ofValue:(NSString*)columnValue
{
    NSString* sql = [[NSString alloc] initWithFormat:@"UPDATE %@ SET %@ = '%@' WHERE %@ = '%@'",table,updatedColumn,value,column,columnValue];
    return sql;
}
